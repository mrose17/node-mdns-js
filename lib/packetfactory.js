...
module.exports.buildANPacket = function (ttl) {
  ...
  packet.push('an', new DNSRecord(
    '_services._dns-sd._udp.' + domain, DNSRecord.Type.PTR, cl, ttl,
    DNSRecord.toName(serviceType)));
  ...
  if (this.options.additionalAddresses) {
    for (var i = 0; i < this.options.additionalAddresses.length; i++) {
      var address = this.options.additionalAddresses[i]
      if (address.indexOf(':') === -1) {
        debug('add additional A record: %s', address);
        packet.push('ar', new DNSRecord(
          target, DNSRecord.Type.A, cl, ttl, DNSRecord.toA(address)));
      } else {
        // TODO: also publish the ip6_address in an AAAA record
